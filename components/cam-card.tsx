...
export const CamCard: React.FC<CamCardProps> = React.memo(({ item }) => {
  ...
  return (
    <Link href={`/cam/${item.id}`} asChild>
      <Pressable className={`flex flex-col overflow-hidden rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-md mb-4 ${isOffline ? 'opacity-80' : ''}`}>
        {/* Image Container */}
